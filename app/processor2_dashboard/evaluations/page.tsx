...
export default async function Processor2EvaluationsPage({
  searchParams,
}: {
  ...
  const referralSyncQs =
    typeof searchParams?.referralSync === "string" && searchParams.referralSync.trim()
      ...
      : "";
  const showAllHref = `/processor2_dashboard/evaluations${referralSyncQs}`;
  ...
  const subs = query
    ...
    : allSubs;
  const downloadedCount = subs.filter((s) => isProcessor2EvalPdfDownloadCurrent(s) && s.processor2PdfDownloadedAt)
    .length;
  ...
  return (
    <main style={{ maxWidth: 1100, margin: "0 auto", padding: "48px 20px" }}>
      ...
      <p style={{ color: "#475569", fontSize: 14, marginTop: 8, lineHeight: 1.55, maxWidth: 900 }}>
        This tab lists client-submitted 1701A evaluations so you can download its PDF and open each record.
      </p>
      <div
        style={{
          marginTop: 12,
      ...
      <section style={{ marginTop: 12, display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8 }}>
        ...
        >
          <span style={{ fontSize: 12, color: "#475569", fontWeight: 700, letterSpacing: "0.01em" }}>Not yet downloaded</span>
      ...
      {searchParams?.downloadAll === "none" ? (
        ...
        >
          Nothing to download: every submission is already marked as downloaded on processor 2 for its current submit version.
        </p>
      ) : null}
          ...
          <div key={s.id} style={card}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div>
                <div style={{ fontWeight: 900, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  ...
                  {s.evaluation.submit1701aCount > 1 ? (
                    <span title="Client resubmitted this evaluation" aria-label="Resubmitted" style={{ display: "inline-flex", color: "#b45309" }}>
                      <ResubmitNoticeIcon />
                    </span>
                  ) : null}
                  {isProcessor2EvalPdfDownloadCurrent(s) && s.processor2PdfDownloadedAt ? (
                    <span
                      title={`PDF fetched ${formatPhilippineDateTime(s.processor2PdfDownloadedAt)} (PH time)`}
                      style={{
                        marginLeft: 6,
                        fontSize: 12,
                        fontWeight: 700,
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <Link href={`/processor2_dashboard/evaluations/${s.evaluationId}`}>View</Link>
                <a href={`/processor2_dashboard/evaluations/${s.evaluationId}/pdf`} style={{ fontWeight: 800 }}>
                  Download PDF
                </a>
              </div>
            </div>
