...
export default async function Processor1EvaluationDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main style={{ maxWidth: 1000, margin: "0 auto", padding: "48px 20px" }}>
      ...
      <p style={{ marginTop: 10, display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10 }}>
        <a href={`/processor1_dashboard/evaluations/${evaluationId}/pdf`} style={{ fontWeight: 900 }}>
          Download PDF
        </a>
        {isProcessor1EvalPdfDownloadCurrent(sub) && sub.processor1PdfDownloadedAt ? (
          <span
            style={{
              fontSize: 13,
              fontWeight: 700,
            ...
            title={new Date(sub.processor1PdfDownloadedAt).toLocaleString()}
          >
            Downloaded
          </span>
        ) : null}
