...
/** Package `code` for admin-quoted PayMongo orders (see `getQuotedBillingPackageId`). */
export const QUOTED_BILLING_CODE = "QUOTED_BILLING";
...
export async function getQuotedBillingPackageId(): Promise<string> {
  const existing = await prisma.servicePackage.findFirst({
    where: { code: QUOTED_BILLING_CODE },
    select: { id: true },
  });
